...
| --- | --- |
| `tests/test_frontend_contract.py` | Four approved HTML documents; required unified workflow controls; device-local history; no password input; approved API references; visible disclaimer, demonstration, and missing-is-not-low language; JavaScript syntax when Node is available |
| `tests/test_fuzzy.py` | Triangular/trapezoidal memberships, complete bounded result, 27 generated rules, monotonicity, activated rules, missing-input incomplete behavior, low/very-high model span, and rejection of threshold gaps |
...
| `tests/test_importer.py` | Stored `0â€“1` fraction/provenance, invalid fraction rejection/error file, EPSG:3857 reprojection, demonstration-only seed labels, and prevention of a demo layer replacing an official hazard slug |
...
| `tests/test_workflow.py` | Boundary/barangay identification, official-boundary preference, complete and incomplete assessments, explanation persistence, outside rejection, private-token access, complete/incomplete PDF behavior, report records, and disabled shared history |
...
### AC-UI-001 â€” Unified unauthenticated interface
...
### AC-UI-002 â€” Map and boundaries
...
### AC-UI-005 â€” Map-click selection
...
### AC-UI-006 â€” Basey containment and barangay
...
### AC-DATA-001 â€” Three required hazards
...
### AC-DATA-002 â€” Normalization semantics

For verified live sources, model `0.5.1-demo` must perform the exact
...
```text
model input = stored normalized fraction Ã— 100
```
...
- Inspection: `config/fuzzy_model.json` version `0.5.1-demo`,
  [fuzzy-data-transformations.md](fuzzy-data-transformations.md), and import
  documentation.

### AC-DATA-003 â€” Hazard visualization
...
### AC-DATA-004 â€” Historical incidents
...
### AC-DATA-006 â€” Official versus demonstration
...
### AC-DATA-007 â€” Live ULAP integrity
...
### AC-FIS-001 â€” Model configuration

`config/fuzzy_model.json` must identify `0.5.1-demo` and define:

- exact live-code/model `0â€“100` transformations, imported `0â€“1`
...
- output memberships;
- Low 1â€“25, Moderate 26â€“50, High 51â€“75, and Very High 76â€“100 thresholds;
...
### AC-FIS-003 â€” Missing required data
...
### AC-FIS-004 â€” Demonstration validation status
...
### AC-OUT-001 â€” Results and recommendations
...
### AC-OUT-005 â€” Methodology and sources
...
### AC-IMP-002 â€” Validation, provenance, and errors

The importer must validate required fields, coordinates, geometry structure, and `0â€“1` hazard fractions; perform only documented safe repairs; preserve provenance/checksum/CRS/quality metadata; and write rejected feature records outside the application UI.
...
### AC-DB-001 â€” Approved table allowlist
...
### AC-DB-002 â€” Immutable assessment snapshot
...
### AC-API-001 â€” Approved API allowlist
...
### AC-SCOPE-001 â€” No identity or RBAC
...
### AC-SCOPE-002 â€” No administrative subsystem
...
### AC-SCOPE-003 â€” No audit-management feature
...
Automated evidence for AC-API-001 and AC-SCOPE-001â€“003:
